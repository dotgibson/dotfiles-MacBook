...
LINKS_ONLY=0; NO_BREW=0; RUN_DEFAULTS=0
...
[[ -d "$REPO/core" ]] || { err "core/ subtree missing — run: git subtree add --prefix=core <dotfiles-core-url> main --squash"; exit 1; }

# ── link helper: back up a real file once, then symlink ──────────────────────
link(){ # link <src> <dest>
  local src="$1" dest="$2"
  [[ -e "$src" ]] || { info "skip (missing): ${src#$REPO/}"; return 0; }
  mkdir -p "$(dirname "$dest")"
  if [[ -L "$dest" ]]; then rm -f "$dest"
  elif [[ -e "$dest" ]]; then mv "$dest" "$dest.pre-dotfiles.$(date +%Y%m%d-%H%M%S)"; info "backed up existing $dest"; fi
  ln -s "$src" "$dest"; ok "${dest/#$HOME/~}"
}

# ── provision (Homebrew + packages) ──────────────────────────────────────────
provision(){
  if ! xcode-select -p >/dev/null 2>&1; then
    say "Xcode Command Line Tools"; xcode-select --install 2>/dev/null || true
  ...
  if [[ "$NO_BREW" == 0 && -f "$REPO/Brewfile" ]]; then
    say "brew bundle (this can take a while)"; brew bundle --file="$REPO/Brewfile"
  else
...
wire_links(){
  ...
  link "$REPO/core/tmux/scripts"   "$CFG/tmux/scripts"            # popup scripts (prefix w/T/f)
  chmod +x "$REPO"/core/tmux/scripts/*.sh 2>/dev/null || true     # ensure popups are runnable
  link "$REPO/os/macos.conf"       "$CFG/tmux/os.conf"            # en0 + battery (sourced by tmux.conf)
  ...
  if [[ ! -e "$CFG/git/local.gitconfig" ]]; then
    mkdir -p "$CFG/git"; cp "$REPO/core/git/local.gitconfig.example" "$CFG/git/local.gitconfig"
  ...
  if [[ -f "$REPO/ssh/config" ]]; then
    link "$REPO/ssh/config" "$HOME/.ssh/config"; chmod 600 "$REPO/ssh/config" 2>/dev/null || true
  else
...
if command -v mise >/dev/null 2>&1; then
  say "mise install"; mise install || info "mise install hit an issue — run it manually later"
fi

# macОS system defaults (opt-in: changes system prefs, may need logout)
if (( RUN_DEFAULTS )) && [[ -f "$REPO/macos/defaults.sh" ]]; then
  say "macos/defaults.sh"; bash "$REPO/macos/defaults.sh" || info "defaults.sh hit an issue"
